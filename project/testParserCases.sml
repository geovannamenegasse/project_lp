val cases =
  ...
  (
    let val s = "fun f(Int x, Int b) = match b with | true -> {x + 1} | _ -> x end; f(3,2)"
        (* val e = Let("f", Anon (ListT [IntT, BoolT],"$list",Let("x",Item (1,Var "$list"), Let ("b",Item (2,Var "$list"),Match(Var "b", [(SOME (ConB true), Prim2 ("+",Var "x",ConI 1)),(NONE, Var "x")])))),Call (Var "f",List [ConI 3, ConB true])) *)
    in
        (s, run(fromString s))
    end
  ...
  (
    let val s = "fun f(Int x, Int b) = match b with | 2 -> {x + 1} | 3 -> {false} | _ -> x end; f(3,2)"
        (* val e = Let("f", Anon (ListT [IntT, BoolT],"$list",Let("x",Item (1,Var "$list"), Let ("b",Item (2,Var "$list"),Match(Var "b", [(SOME (ConB true), Prim2 ("+",Var "x",ConI 1)),(NONE, Var "x")])))),Call (Var "f",List [ConI 3, ConB true])) *)
    in
        (s, run(fromString s))
    end
  ...
  (
    let val s = "if 3 = 2 then false else 1 + 4"
        (* val e = If (Prim2 ("=",ConI 3,ConI 2),ConI 0,Prim2 ("+",ConI 1,ConI 4)) *)
    in
        (s, run(fromString s))
    end
  ...
  (
    let val s = "fun f(Int x, Int y) = x - y; f(true,4)"
        (* val e = Let ("f", Anon (ListT [IntT, IntT], "$list", Let ("x",Item (1, Var "$list"), Let ("y",Item (2, Var "$list"),Prim2 ("-",Var "x",Var "y")))), Call (Var "f",List [ConI 5, ConI 4])) *)
    in
        (s, run(fromString s))
    end
  ...
  (
    let val s = "fun rec f (Int x) : Bool = if x <= 0 then 1 else x; f(5)"
        (* val e = Letrec ("f",IntT,"x", IntT, If (Prim2 ("<=",Var "x",ConI 0),ConI 1, Prim2 ("+",Var "x",Call (Var "f",Prim2 ("-",Var "x",ConI 1)))), Call (Var "f",ConI 5)) *)
    in
        (s, run(fromString s))
    end
  ...
  (
    let val s = "fun f(Int x, Int y) = x - y; var x = 2; x(5,4)"
        (* val e = Let ("f", Anon (ListT [IntT, IntT], "$list", Let ("x",Item (1, Var "$list"), Let ("y",Item (2, Var "$list"),Prim2 ("-",Var "x",Var "y")))), Call (Var "f",List [ConI 5, ConI 4])) *)
    in
        (s, run(fromString s))
    end
  ...
  [(
    let val s = "fun f(Int x, Int b) = match b with | 2 -> {x + 1} | 3 -> {x + 2} end; f(3,7)"
        (* val e = Match (Var "x",[(SOME (ConI 0), ConI 1), (NONE, Prim1 ("-",ConI 1))]) *)
    in
        (s, run(fromString s))
    end
